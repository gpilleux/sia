...
class AutoDiscovery:
    ...
    def _find_directory_recursive(
        self, target_name: str, max_depth: int = 4, exclude_patterns: List[str] = None
    ) -> Optional[Path]:
    ...
    def extract_bounded_contexts(self):
        ...
        domain_dir = self._find_directory_recursive(
            "domain", exclude_patterns=["test", "tests", "testing"]
        )
        ...
        if not contexts:
            ...
            api_dir = self._find_directory_recursive(
                "api", exclude_patterns=["test", "tests", "testing"]
            )
            ...
            if api_dir:
                ...
                for subdir_name in ["v1", "routers", "routes"]:
                    ...
                    if routes_dir.exists():
                        print(
                            f"   🔍 Found API directory: {routes_dir.relative_to(self.root)}"
                        )
        ...
        for agents_dir in [sia_agents_dir, legacy_agents_dir]:
            if agents_dir.exists():
                for agent_file in agents_dir.glob("*.md"):
                    if (
                        agent_file.name == f"{project_name}.md"
                        or agent_file.name == "README.md"
                    ):
                        continue
    ...
    def assemble_instructions(self):
        ...
        content = content.replace(
            "{{PROJECT_TYPE}}", self.config["project"].get("type", "Unknown")
        )
        content = content.replace(
            "{{BOUNDED_CONTEXTS}}",
            ", ".join(self.config["domain"].get("bounded_contexts", [])),
        )
        ...
        content = content.replace(
            "{{REQUIREMENTS_STATUS}}", "System initialized. Ready for requirements."
        )
